...
const BookingCabInfoBox: React.FC<ICabBox> = ({ exclusions, facilities, inclusions, taq, className }) => {
    ...
    const classTab = "w-full h-full uppercase   select-none cursor-pointer flex justify-center items-center text-[10px] font-bold";
    return (
        <div className={" transition-all  rounded-xl overflow-hidden " + className}>
...
export function BookingWrapper({ children, className }: MyComponentProps) {
    return (
        <UserWrapper className="bg-orange-50 pt-11 ">
            <ContainerWrapper className={"grid grid-cols-1 md:grid-cols-12  gap-10 pb-56 " + className}>
                {children}
            </ContainerWrapper>
        </UserWrapper>
    )
