...
import { addNewRoundTripCabExclusionsData, removeNewRoundTripCabExclusionsData, setNewRoundTripCabData, setNewRoundTripCabExclusionsData } from "../../../../../store/provider/addCabs/AddRoundTripCabProvider";
...
function AddRoundTripStep3() {
    const data = useSelector((state: RootState) => state.addroundtripcab);
    const dispatch = useDispatch<AppDispatch>();

    const addmore = () => {
        dispatch(addNewRoundTripCabExclusionsData());
    };

    const remove = (i: number) => {
        dispatch(removeNewRoundTripCabExclusionsData({ index: i }));
    };

    const chengeData = (index: number, key: ExclusionsKeys, value: number | string) => {
        dispatch(setNewRoundTripCabExclusionsData({ index, key, value }));
    };
    ...
    const goNextStep = () => {
        if (!areServiceValuesPresent(data.exclusions)) {
            ...
            dispatch(setNewRoundTripCabData({ name: "step", value: 3 }))
        }
    };
...
export default AddRoundTripStep3