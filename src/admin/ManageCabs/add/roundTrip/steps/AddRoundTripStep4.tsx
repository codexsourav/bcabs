...
import { addNewRoundTripCabFacilitiesData, removeNewRoundTripCabFacilitiesData, setNewRoundTripCabData, setNewRoundTripCabFacilitiesData } from "../../../../../store/provider/addCabs/AddRoundTripCabProvider";
...
function AddRoundTripStep4() {
    const data = useSelector((state: RootState) => state.addroundtripcab);
    const dispatch = useDispatch<AppDispatch>();

    const addmore = () => {
        dispatch(addNewRoundTripCabFacilitiesData());
    };

    const remove = (i: number) => {
        dispatch(removeNewRoundTripCabFacilitiesData({ index: i }));
    };

    const chengeData = (index: number, key: FacilitiesKeys, value: number | string) => {
        dispatch(setNewRoundTripCabFacilitiesData({ index, key, value }));
    };
    ...
    const goNextStep = () => {
        if (!areServiceValuesPresent(data.facilities)) {
            ...
            dispatch(setNewRoundTripCabData({ name: "step", value: 4 }))
        }
    };
...
export default AddRoundTripStep4