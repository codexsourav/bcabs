...
import { addNewRoundTripCabTAQData, removeNewRoundTripCabTAQData, setNewRoundTripCabData, setNewRoundTripCabTAQData } from "../../../../../store/provider/addCabs/AddRoundTripCabProvider";
...
function AddRoundTripStep5() {
    const data = useSelector((state: RootState) => state.addroundtripcab);
    const dispatch = useDispatch<AppDispatch>();

    const addmore = () => {
        dispatch(addNewRoundTripCabTAQData());
    };

    const remove = (i: number) => {
        dispatch(removeNewRoundTripCabTAQData({ index: i }));
    };

    const chengeData = (index: number, value: string) => {
        dispatch(setNewRoundTripCabTAQData({ index, value }));
    };
    ...
    const goNextStep = () => {
        if (!areAllValuesPresent(data.taq)) {
            ...
            dispatch(setNewRoundTripCabData({ name: "step", value: 5 }))
        }
    };
...
export default AddRoundTripStep5