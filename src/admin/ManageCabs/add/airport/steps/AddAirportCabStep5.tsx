...
import { addNewAirportCabTAQData, removeNewAirportCabTAQData, setNewAirportCabData, setNewAirportCabTAQData } from "../../../../../store/provider/addCabs/AddAirportCabProvider";

function AddAirportCabStep5() {
    const data = useSelector((state: RootState) => state.addairportcab);
    const dispatch = useDispatch<AppDispatch>();

    const addmore = () => {
        dispatch(addNewAirportCabTAQData());
    };

    const remove = (i: number) => {
        dispatch(removeNewAirportCabTAQData({ index: i }));
    };

    const chengeData = (index: number, value: string) => {
        dispatch(setNewAirportCabTAQData({ index, value }));
    };
    ...
    const goNextStep = () => {
        if (!areAllValuesPresent(data.taq)) {
            ...
            dispatch(setNewAirportCabData({ name: "step", value: 5 }))
        }
    };
...
export default AddAirportCabStep5;