...
import { addNewAirportCabInclusionsData, removeNewAirportCabExclusionsData, setNewAirportCabData, setNewAirportCabInclusionsData } from "../../../../../store/provider/addCabs/AddAirportCabProvider";

function AddAirportCabStep2() {
    const data = useSelector((state: RootState) => state.addairportcab);
    const dispatch = useDispatch<AppDispatch>();
    const addmore = () => {
        dispatch(addNewAirportCabInclusionsData());
    };
    ...
    const changeData = (index: number, key: InclusionsKeys, value: number | string) => {
        dispatch(setNewAirportCabInclusionsData({ index, key, value }));
    };
    ...
    const goNextStep = () => {
        if (!areServiceValuesPresent(data.inclusions)) {
            ...
            dispatch(setNewAirportCabData({ name: "step", value: 2 }))
        }
    };
...
export default AddAirportCabStep2