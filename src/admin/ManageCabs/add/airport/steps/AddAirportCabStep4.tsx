...
import { addNewAirportCabFacilitiesData, removeNewAirportCabFacilitiesData, setNewAirportCabData, setNewAirportCabFacilitiesData } from "../../../../../store/provider/addCabs/AddAirportCabProvider";

function AddAirportCabStep4() {
    const data = useSelector((state: RootState) => state.addairportcab);
    const dispatch = useDispatch<AppDispatch>();

    const addmore = () => {
        dispatch(addNewAirportCabFacilitiesData());
    };

    const remove = (i: number) => {
        dispatch(removeNewAirportCabFacilitiesData({ index: i }));
    };

    const chengeData = (index: number, key: FacilitiesKeys, value: number | string) => {
        dispatch(setNewAirportCabFacilitiesData({ index, key, value }));
    };
    ...
    const goNextStep = () => {
        if (!areServiceValuesPresent(data.facilities)) {
            ...
            dispatch(setNewAirportCabData({ name: "step", value: 4 }))
        }
    };
...
export default AddAirportCabStep4;