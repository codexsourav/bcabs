...
import { addNewOneWayCabExclusionsData, removeNewOneWayCabExclusionsData, setNewOneWayCabData, setNewOneWayCabExclusionsData } from "../../../../../store/provider/addCabs/AddOneWayCabProvider";
import { areServiceValuesPresent } from "../../../../../utils/mapOptons";
import { toast } from "react-toastify";

function OneWayCabAddStep3() {
    const data = useSelector((state: RootState) => state.addonewaycab);
    const dispatch = useDispatch<AppDispatch>();

    const addmore = () => {
        dispatch(addNewOneWayCabExclusionsData());
    };

    const remove = (i: number) => {
        dispatch(removeNewOneWayCabExclusionsData({ index: i }));
    };

    const chengeData = (index: number, key: InclusionsKeys, value: number | string) => {
        dispatch(setNewOneWayCabExclusionsData({ index, key, value }));
    };
    ...
    const goNextStep = () => {
        if (!areServiceValuesPresent(data.exclusions)) {
            ...
            dispatch(setNewOneWayCabData({ name: "step", value: 3 }))
        }
    };
...
export default OneWayCabAddStep3;