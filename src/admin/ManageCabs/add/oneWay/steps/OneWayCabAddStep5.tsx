...
import { addNewOneWayCabTAQData, removeNewOneWayCabTAQData, setNewOneWayCabData, setNewOneWayCabTAQData } from "../../../../../store/provider/addCabs/AddOneWayCabProvider";
import { areAllValuesPresent } from "../../../../../utils/mapOptons";
import { toast } from "react-toastify";

function OneWayCabAddStep5() {
    const data = useSelector((state: RootState) => state.addonewaycab);
    const dispatch = useDispatch<AppDispatch>();

    const addmore = () => {
        dispatch(addNewOneWayCabTAQData());
    };

    const remove = (i: number) => {
        dispatch(removeNewOneWayCabTAQData({ index: i }));
    };

    const chengeData = (index: number, value: string) => {
        dispatch(setNewOneWayCabTAQData({ index, value }));
    };
    ...
    const goNextStep = () => {
        if (!areAllValuesPresent(data.taq)) {
            ...
            dispatch(setNewOneWayCabData({ name: "step", value: 5 }))
        }
    };
...
export default OneWayCabAddStep5;