...
import { addNewLocalCabExclusionsData, removeNewLocalCabExclusionsData, setNewLocalCabData, setNewLocalCabExclusionsData } from "../../../../../store/provider/addCabs/AddLocalCabProvider";
...
function AddLocalStep3() {
    const data = useSelector((state: RootState) => state.addlocalcab);
    const dispatch = useDispatch<AppDispatch>();

    const addmore = () => {
        dispatch(addNewLocalCabExclusionsData());
    };

    const remove = (i: number) => {
        dispatch(removeNewLocalCabExclusionsData({ index: i }));
    };

    const chengeData = (index: number, key: ExclusionsKeys, value: number | string) => {
        dispatch(setNewLocalCabExclusionsData({ index, key, value }));
    };
    ...
    const goNextStep = () => {
        if (!areServiceValuesPresent(data.exclusions)) {
            ...
            dispatch(setNewLocalCabData({ name: "step", value: 3 }))
        }
    };
